...
GOOGLE_CLIENT_ID = "125994508307-sq45866e7ivji4cheujh0vipg46mj3qj.apps.googleusercontent.com"
...
app.config['MONGO_URI'] = 'mongodb+srv://resumeparser:resumeparser@cluster0.lc1st.mongodb.net/ResumeRanking'
mongo = PyMongo(app)
...
nlp = spacy.load(r'C:\Users\prakhar.pathrikar\PycharmProjects\projects\git_spacy_resume_rank\ner_models\ResumeModel\output\model-best')
...
def HR():
    if request.method == 'POST':
        ...
        #Creating HR user list
        hr_users = ["PRATIK", "PRAKHAR"]
        # Check the user's response and route accordingly
        if response in hr_users:
            return render_template("CompanyDashboard.html")
        else:
